    ...
    startTransition(async () => {
      try {
        ...
        if (signInError) {
          ...
          if (signInError.message?.includes('NetworkError') || signInError.message?.includes('Failed to fetch')) {
            setError('Erro de conexão: Não conseguir conectar ao Supabase. Verifique:\n1. Sua conexão com a internet\n2. As variáveis NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY no .env.local\n3. Se seu projeto Supabase está ativo')
        ...
        const message = err instanceof Error ? err.message : 'Erro desconhecido ao entrar'
